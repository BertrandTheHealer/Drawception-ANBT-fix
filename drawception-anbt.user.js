...
const wrapped = () => {
  ...
  */
  ...
  const addScriptSettings = () => {
    ...
    addGroup('Miscellaneous', [['localeTimestamp', 'boolean', `Format timestamps as your system locale (${new Date().toLocaleString()})`], ['proxyImgur', 'boolean', 'Replace imgur.com links to filmot.com to load, in case your ISP blocks them'], ['ajaxRetry', 'boolean', 'Retry failed AJAX requests'], ['autoplay', 'boolean', 'Automatically start replay when watching playback'], ['autoBypassNSFW', 'boolean', 'Automatically bypass NSFW game warning'], ['markStalePosts', 'boolean', 'Mark stale forum posts'], ['maxCommentHeight', 'number', 'Maximum comments and posts height until directly linked (px, 0 = no limit)'], ['useOldFont', 'boolean', 'Use old Nunito font (which is usually bolder and less wiggly)'], ['useOldFontSize', 'boolean', 'Use old, smaller font size'],["colorizeNavBar", "boolean", "Change top bar color based on panel colors"],["checkForNotifications", "boolean", "Check for notifications periodically while page is open"],["hideAnnouncements", "boolean", "Hide announcements and featured games banner on homepage"]])
  ...
  const pageEnhancements = () => {
    ...
    if (options.hideAnnouncements) {
      GM_addStyle("#header-bar-container { display: none; }");
    }
  ...
  try{
    if(options.checkForNotifications){
  ...
  function launchNotifications(notification, unreadNotifications) {
    $("#btn-notifications").click();//open unread menu
  }
  ...
  function checkNotifications(){
      //load new notification number
      var unreadCount = "";
      var xhttp = new XMLHttpRequest();
      xhttp.onreadystatechange = function() {
       if (this.readyState == 4 && this.status == 200) {
         parser = new DOMParser();
         doc = parser.parseFromString(xhttp.responseText, "text/html");//parse the homepage
         unreadCount = doc.getElementsByTagName("btn-notifications")[0].getAttribute(":amount");//doc.getElementById("user-notify-count").innerHTML;//get unread count
         console.log(unreadCount+" new notifications");
         if(parseInt(unreadCount)>0){//if there are new notifications
          $('#user-notify-count').innerHTML = unreadCount;
          $('#btn-notifications').classList.add("btn-danger");
          $('#btn-notifications').classList.remove("btn-menu");
          GM_addStyle(`#user-notify-list .list-group .list-group-item .fas {color: #888}#user-notify-list .list-group .list-group-item:nth-child(-n+${unreadCount}) .fas {color: #2F5}a.wrong-order {color: #F99} div.comment-holder:target {background-color: #DFD}.comment-new a.text-muted:last-child:after {content: 'New'; color: #2F5; font-weight: bold; background-color: #183; border-radius: 9px; display: inline-block; padding: 0px 6px; margin-left: 10px;}`);
          if(window.Notification){//and we have notification permissions
            ...
            notification.onclick = function() {//when the notification is activated
              launchNotifications(notification, unreadCount);//run function
      ...
      xhttp.open("GET", "https://drawception.com/", true);
      xhttp.send();
}
  ...
  if(options.colorizeNavBar){
    ...
    try{//if the game is not a vet game the theme will be in the first "label-no-select" element
      ...
      var theme = document.getElementsByClassName("label-big")[0].firstChild.innerHTML;
      switch(theme) {
        case "b &amp; w":
          navbarColor = "#000000";
          break;
        ...
        case "grimby grays":
          ...
          break;
        case "juice":
          navbarColor = "rgb(243, 171, 84)";
          break;
        case "tropical":
          navbarColor = "rgb(246, 131, 87)";
          break;
        case "fire &amp; ice":
          navbarColor = "#FD2119";
          break;
      ...
      if(navbarColor != "#0CE853"){
        ...
        $("#nav-drag").style.background = "";
      }
      $("#nav-drag").style.backgroundColor = navbarColor;
    }catch(error){}
    try{//if the game is a vet game the theme will be in the second "label-no-select" element
      var navbarColor = "#0CE853";
      var theme = document.getElementsByClassName("label-big")[1].firstChild.innerHTML;
      switch(theme) {
        case "b &amp; w":
          navbarColor = "#000000";
          break;
